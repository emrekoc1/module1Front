...
@Component({
    selector: 'app-machine-stop-add',
    templateUrl: './add.html',
    styleUrls: ['./add.scss'],
    animations: fuseAnimations,
    encapsulation: ViewEncapsulation.None
})
export class AddMachineStopComponent implements OnInit {
    ...
    async ngOnInit() {
        await this.stop_service.get_all_stop_category().then(result => {
            console.log(result);
            this.data_all_stop_category = result;
        })
    ...
    async add_stop_reason_for_machine() {
        ...
        this.stop_service.add_reason_to_machine(data).then(result=>{
    ...
    async get_reason_by_category(value){
        console.log(value);
        var selected_stop_category=this.data_all_stop_category.find(a=>a._id===value);
